...
ggplot(df, aes(x = avg_agi, y = services_per_beneficiary, color = income_group)) +
...
ggplot(df, aes(x = avg_agi, y = payment_per_beneficiary, color = income_group)) +
  geom_point(alpha = 0.6) +
  theme_minimal() +
  labs(
...
df$income_group <- factor(df$income_group, levels = c("Lower Class", "Lower Middle", "True Middle", "Upper Middle", "Upper Class"))

anova_services <- aov(services_per_beneficiary ~ income_group, data = df)
anova_payments <- aov(payment_per_beneficiary ~ income_group, data = df)
