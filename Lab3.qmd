...
# Extract the usernames (portion before @)
usernames <- sapply(emails, "[[", 1)

# Detect non-alphanumeric characters in the usernames
username_non_alphanumeric <- grepl("[^a-zA-Z0-9]", usernames)

# Extract the domains (portion after @)
domains <- sapply(emails, "[[", 2)

# Remove the top-level domain (TLD), keeping only the domain name
domains <- gsub("\\.[a-z]{2,}$", "", domains)

# Detect non-alphanumeric characters in the domains
...
# take first 3 digs of phone numebrs 
...
# Extract the apartment number by splitting the addresses and taking the last element
numbers <- sapply(strsplit(apartments, " "), function(x) x[length(x)])

# Remove any "#" symbols and convert the numbers to numeric
numbers <- as.numeric(gsub("#", "", numbers))

# Plot the histogram of the log of the apartment numbers
hist(log(numbers), 
     main = "Histogram of Log of Apartment Numbers", 
