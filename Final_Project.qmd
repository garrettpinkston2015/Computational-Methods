...
income_quantiles <- quantile(df$avg_agi, probs = c(0.33, 0.66), na.rm = TRUE)

df <- df %>%
  mutate(income_group = case_when(
    avg_agi <= income_quantiles[1] ~ "Low",
    avg_agi > income_quantiles[1] & avg_agi <= income_quantiles[2] ~ "Middle",
    avg_agi > income_quantiles[2] ~ "High",
    TRUE ~ NA_character_
  ))
...
t_test_services <- t.test(
  df$services_per_beneficiary[df$income_group == "High"],
  df$services_per_beneficiary[df$income_group == "Low"],
  na.rm = TRUE
)

#high vs low payment per beneficiary - Payment per Beneficiary
t_test_payments <- t.test(
  df$payment_per_beneficiary[df$income_group == "High"],
  df$payment_per_beneficiary[df$income_group == "Low"],
  na.rm = TRUE
)
